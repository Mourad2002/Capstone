#include "Drone.h"
#include "DataCollector.h"  // Ensure DataCollector is included
...
void Drone::update(double dt) {
  static DataCollector& collector = DataCollector::getInstance();
  collector.updateTotalTime(dt);

  if (available) getNextDelivery();

  if (toPackage) {
    collector.getInitTime();
  ...
  if (toFinalDestination && pickedUp) {
    ...
    if (toFinalDestination->isCompleted()) {
      collector.getFinalTime();
      collector.collectData(package->getStrategyName());
